...
export const myOrders = () => async (dispatch) => {
  try {
    ...
    const { data } = await API.get("/api/v1/orders/me", config);
...
export const deleteOrder = (id) => async (dispatch) => {
  try {
    ...
    const { data } = await API.delete(`/api/v1/admin/order/${id}`, config);
...
export const getOrderDetails = (id) => async (dispatch) => {
  try {
    ...
    const { data } = await API.get(`/api/v1/order/${id}`, config);
